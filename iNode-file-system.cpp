	...
	while(1)
	{
		...
		else if(choice == 7)
		{
			...
			for ( it = dir_map.begin(); it!=dir_map.end(); it++) {				
					printf("\033[1;32m%s   %d KB\033[0m\n",(it->first).c_str(),inode_arr[it->second].filesize/1024);
			}
		}
		else if(choice == 8)
		{
...
int create_disk()
{
	...
	fwrite(inode_buff,1,sizeof(inode_arr),fp);
  	...
  	fclose(fp);
  	printf("\033[1;32mVirtual Disk '%s' of size %d MB created sucessfully\033[0m\n",disk_name, DISK_BLOCKS*BLOCK_SIZE/(1000*1000) );
	...
	if ( dir_map.find(string(name)) == dir_map.end() )
	{
 	...
 	else
 	{
 		...
 		else
 		{
 			...
 			printf("\033[1;32mGiven %d as File descriptor\033[0m\n",fd);
...
int close_file(int fd)
{
	if ( fileDescriptor_map.find(fd) == fileDescriptor_map.end() )
	{
 	...
 	else
 	{
		...
		printf("\033[1,32m File Descripter %d closed successfully\033[0m\n",fd);
...
int delete_file(char * filename)
{
	...
	printf("\033[1;32mFile is deleted successfully.\033[0m\n");
	return 0;
...
int store_file_into_Disk(char * filename)
{
	...
	close_file(fd2);
		printf("\033[1,32mFile written Successfully\033[0m\n");
